...
public class DfeClassLoader extends URLClassLoader {
	...
	public DfeClassLoader(String[] paths, ClassLoader parent)
	...
	public Class<?> findClass(String name) throws ClassNotFoundException {
		...
		try (InputStream in = super.getResourceAsStream(path)) {
			...
			throw new ClassNotFoundException(
					"I/O exception while loading class.", e);
		}
		...
		byte[] bytesToLoad;
		try {
			byte[] instrumented = transformer.transform(this, internalName,
	...
	public Class<?> testDefineClass(byte[] classBytes) {
		return defineClass("edu.purdue.dfe.TestSample", classBytes, 0,
				classBytes.length);
	}
}
