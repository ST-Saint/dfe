...
public class InstructionClassAdapter extends ClassVisitor {
	...
	private static final int probeAcc = Opcodes.ACC_SYNTHETIC
	...
	private static final int clazzAcc = Opcodes.ACC_PRIVATE
			| Opcodes.ACC_STATIC;
	...
	public final MethodVisitor visitMethod(final int access, final String name,
		...
		if (mv == null) {
			...
			return new InstructionMethodAdapter(mv, cv, clazzName, name, desc,
					signature, exceptions);
		}
	}

	private void createInitMethod(final ClassVisitor cv, final int probeCount) {
		final MethodVisitor mv = cv.visitMethod(INITMETHOD_ACC, INITMETHOD_NAME,
				INITMETHOD_DESC, null, null);
		mv.visitCode();

		// Load the value of the static data field:
		mv.visitFieldInsn(Opcodes.GETSTATIC, className, probeFieldName,
				probeDesc);
	...
	private int genInitializeDataField(final MethodVisitor mv,
		...
		final int size = systemPropertyDataAccessor(classId, className,
				probeCount, mv);
		...
		mv.visitFieldInsn(Opcodes.PUTSTATIC, className, probeFieldName,
				probeDesc);
	...
	private int systemPropertyDataAccessor(Long classId, String className,
	...
	private void createDataField(final ClassVisitor cv) {
		cv.visitField(DfeProperty.DATAFIELD_ACC, DfeProperty.DATAFIELD_NAME,
				DfeProperty.DATAFIELD_DESC, null, null);
	}
	...
	public int storeInstance(final MethodVisitor mv, final boolean clinit,
		...
		mv.visitMethodInsn(Opcodes.INVOKESTATIC, className,
				DfeProperty.INITMETHOD_NAME, DfeProperty.INITMETHOD_DESC,
				false);
