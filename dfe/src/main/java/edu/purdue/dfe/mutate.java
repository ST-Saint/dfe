...
public class mutate {
	...
	private final int[] INTERESTING16 = { -32768, -129, 128, 255, 256, 512,
			1000, 1024, 4096, 32767, -128, -1, 0, 1, 16, 32, 64, 100, 127 };
	private final int[] INTERESTING32 = { -2147483648, -100663046, -32769,
			32768, 65535, 65536, 100663045, 2147483647, -32768, -129, 128, 255,
			256, 512, 1000, 1024, 4096, 32767, -128, -1, 0, 1, 16, 32, 64, 100,
			127 };
	...
	private String dec2bin(int dec) {
		...
		String padding = new String(new char[32 - bin.length()]).replace("\0",
				"0");
		return padding + bin;
	}
	...
	static void copy(byte[] src, int srcPos, byte[] dst, int dstPos,
		...
		System.arraycopy(src, srcPos, dst, dstPos,
				Math.min(length, src.length - srcPos));
	}

	static void copy(byte[] src, int srcPos, byte[] dst, int dstPos) {
		System.arraycopy(src, srcPos, dst, dstPos,
				Math.min(src.length - srcPos, dst.length - dstPos));
	}
	...
	public byte[] mutate(byte[] buf) {
		...
		for (int i = 0; i < nm; i++) {
			...
			if (x == 0) {
				...
				int pos = this.rand(res.length - 1);
				if (this.randBool()) {
					res[pos] = (byte) (INTERESTING16[this
							.rand(INTERESTING16.length)] & 0xFF);
					res[pos + 1] = (byte) ((INTERESTING16[this
							.rand(INTERESTING16.length)] >> 8) & 0xFF);
				} else {
					res[pos + 1] = (byte) (INTERESTING16[this
							.rand(INTERESTING16.length)] & 0xFF);
					res[pos] = (byte) ((INTERESTING16[this
				...
				if (this.randBool()) {
					res[pos] = (byte) (INTERESTING32[this
							.rand(INTERESTING32.length)] & 0xFF);
					res[pos + 1] = (byte) ((INTERESTING32[this
							.rand(INTERESTING32.length)] >> 8) & 0xFF);
					res[pos + 2] = (byte) ((INTERESTING32[this
							.rand(INTERESTING32.length)] >> 16) & 0xFF);
					res[pos + 3] = (byte) ((INTERESTING32[this
							.rand(INTERESTING32.length)] >> 24) & 0xFF);
				} else {
					res[pos + 3] = (byte) (INTERESTING32[this
							.rand(INTERESTING32.length)] & 0xFF);
					res[pos + 2] = (byte) ((INTERESTING32[this
				...
				int idx0 = 0;
				while (idx0 < res.length && idx0 < other.length
						&& res[idx0] == other[idx0]) {
					idx0++;
				}
				int idx1 = 0;
				while (idx1 < res.length && idx1 < other.length
						&& res[res.length - idx1 - 1] == other[other.length
								- idx1 - 1]) {
					idx1++;
				}
				int diff = Math.min(res.length - idx0 - idx1,
						other.length - idx0 - idx1);
				if (diff < 4) {
					i--;
					continue;
				}
				copy(other, idx0, res, idx0,
						Math.min(other.length, idx0 + this.rand(diff - 2) + 1)
								- idx0);
			} else if (x == 15) {
